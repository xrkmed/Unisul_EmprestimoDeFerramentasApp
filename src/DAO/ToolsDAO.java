...
public class ToolsDAO {
    ...
    public ToolModel addTool(String nome, double price, int fabricanteId) throws IllegalArgumentException, DatabaseResultQueryException, SQLException {
        ...
        while (result.next()) {
            ...
            ToolModel tool = new ToolModel(ferramentaId, nome.toUpperCase(), (fabricanteId != -1 ? ManufacturerDAO.getInstance().getManufacturer(fabricanteId) : null), price);
            return tool;
        }
    ...
    public ArrayList<ToolModel> getTools() throws SQLException, DatabaseResultQueryException {
        ...
        ResultSet result = DBQuery.executeQuery("SELECT f.id, f.name, f.price, IFNULL(FHF.fabricante_id, -1) as fabricante_id FROM tb_ferramentas AS f LEFT JOIN ferramenta_has_fabricante FHF ON FHF.ferramenta_id = f.id GROUP BY f.id, FHF.fabricante_id;");
    ...
    public ToolModel getTool(int id) throws SQLException, DatabaseResultQueryException {
        ResultSet result = DBQuery.executeQuery("SELECT f.id, f.name, f.price, IFNULL(FHF.fabricante_id, -1) as fabricante_id FROM tb_ferramentas AS f LEFT JOIN ferramenta_has_fabricante FHF ON FHF.ferramenta_id = f.id WHERE f.id = ? GROUP BY f.id, FHF.fabricante_id;", id);
    ...
    public ToolModel getTool(String name) throws DatabaseResultQueryException, SQLException {
        ResultSet result = DBQuery.executeQuery("SELECT f.id, f.name, f.price, IFNULL(FHF.fabricante_id, -1) as fabricante_id FROM tb_ferramentas AS f LEFT JOIN ferramenta_has_fabricante FHF ON FHF.ferramenta_id = f.id WHERE f.name = ? GROUP BY f.id, FHF.fabricante_id;", name);
    ...
    public ArrayList<ToolModel> getToolsByManufacturer(int manufacturerId) throws DatabaseResultQueryException, SQLException {
        ArrayList<ToolModel> ferramentas = new ArrayList<>();
        ResultSet result = DBQuery.executeQuery("SELECT f.id, f.name, f.price, fhf.fabricante_id, fe.emprestimo_id FROM tb_ferramentas AS f LEFT JOIN ferramenta_has_fabricante fhf ON fhf.ferramenta_id = f.id WHERE fhf.fabricante_id = ?;", manufacturerId);
    ...
    public ArrayList<ToolModel> getToolsWithoutManufacturer() throws DatabaseResultQueryException, SQLException {
        ArrayList<ToolModel> ferramentas = new ArrayList<>();
        ResultSet result = DBQuery.executeQuery("SELECT f.id, f.name, f.price, -1 as fabricante_id, fe.emprestimo_id FROM tb_ferramentas AS f LEFT JOIN ferramenta_has_fabricante fhf ON fhf.ferramenta_id = f.id WHERE fhf.fabricante_id IS NULL;");
